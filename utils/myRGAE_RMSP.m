function y=myRGAE_RMSP(X,SG,lamda,hid, map, epochs, lr, idex)%
...
%     lr=0.01;epochs=200;                   % learning rate and Itermax
    ...
    for epoch=1:epochs
        ...
        for j=1:batch_num
            ...
            % Backward
            [W1,b1,W2,b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2]=trainAE(x,z,x_hat,W1,b1,W2,b2,res,s_G,lr,lamda,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2);
        end
...
function [vt, WorB] = RMSP(WorB_prev, grad, vt_prev, mu, beta, epsilon)
    
    vt = beta*vt_prev + (1-beta)*grad.^2;
    
    WorB = WorB_prev - (mu ./ sqrt(vt + epsilon)).*grad;
end

function [W1_,b1_,W2_,b2_,vt_W1,vt_b1,vt_W2,vt_b2]=trainAE(x,z,x_hat,W1,b1,W2,b2,res,L,lr,lamda,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2)
% Training the network by BP
    ...
    [vt_W2, W2_] = RMSP(W2, delta1*z'/num, vt_prev_W2, lr, beta, epsilon);
    [vt_b2, b2_] = RMSP(b2, delta1*ones(1,num)'/num, vt_prev_b2, lr, beta, epsilon);
%     W2_=W2-lr*delta1*z'/num;
%     b2_=b2-lr*delta1*ones(1,num)'/num;
    
    [vt_W1, W1_] = RMSP(W1, tmp*x'/num, vt_prev_W1, lr, beta, epsilon);
    [vt_b1, b1_] = RMSP(b1, tmp*ones(1,num)'/num, vt_prev_b1, lr, beta, epsilon);
%     W1_=W1-lr*tmp*x'/num;
